	...
	public class OpcSession
	{
		...
		public async Task ExecuteActionsAsync(CancellationToken ct)
		{
			...
			try
			{
				...
				List<OpcAction> currentOpcActions = new List<OpcAction>(OpcActions);
				foreach (var action in currentOpcActions)
				{
					Logger.Information($"Execute '{action.GetType()}' action on node '{action.OpcNodeId}' on endpoint '{EndpointUrl.AbsoluteUri}' {(UseSecurity ? "with" : "without")} security.");
		...
		public NodeId GetNodeIdFromId(string id)
		{
			...
			try
			{
				if (id.Contains("nsu="))
				{
					Logger.Debug($"Normalizing «{id}»...");
					sanitizedId = id.Replace(" ;", ";");
					ExpandedNodeId expandedNodeId = ExpandedNodeId.Parse(sanitizedId);
		...
		{
			var recurringActions
				= OpcActions.Where(a => a.Interval > 0);
			return
				 recurringActions.Count() > 0 ?
					recurringActions.Min(a => a.Interval)
					: (int?)null;
		}
